...
def upgrade() -> None:
    ...
    op.execute("UPDATE users SET is_verified = TRUE, verified_at = created_at WHERE is_verified IS NULL")

    op.alter_column("users", "is_verified", nullable=False)
    op.create_index("ix_users_username_lower_unique", "users", [sa.text("LOWER(username)")], unique=True)
