...
def validate_agent_contributor(value: str) -> str:
    ...
    if len(agent) < consts.LEN_CONTRIB_AGENT_NAME_MIN:
        raise ValueError(
            f"agent name must be at least {consts.LEN_CONTRIB_AGENT_NAME_MIN} characters long"
        )
    if len(agent) > consts.LEN_CONTRIB_AGENT_NAME_MAX:
        raise ValueError(
            f"agent name must be no greater than {consts.LEN_CONTRIB_AGENT_NAME_MAX} characters long"
        )
    if len(host) < consts.LEN_CONTRIB_HOST_NAME_MIN:
        raise ValueError(
            f"host name must be at least {consts.LEN_CONTRIB_HOST_NAME_MIN} characters long"
        )
    if len(host) > consts.LEN_CONTRIB_HOST_NAME_MAX:
        raise ValueError(
            f"host name must be no greater than {consts.LEN_CONTRIB_HOST_NAME_MAX} characters long"
        )
    return value
...
def validate_rfc_title(title: str) -> str:
    if len(title) < consts.LEN_RFC_TITLE_MIN:
        raise ValueError(
            f"title must be at least {consts.LEN_RFC_TITLE_MIN} characters long"
        )
    if len(title) > consts.LEN_RFC_TITLE_MAX:
        raise ValueError(
            f"title must be no greater than {consts.LEN_RFC_TITLE_MAX} characters long"
        )
    return title
...
def validate_rfc_slug(slug: str) -> str:
    if len(slug) < consts.LEN_RFC_SLUG_MIN:
        raise ValueError(
            f"slug must be at least {consts.LEN_RFC_SLUG_MIN} characters long"
        )
    if len(slug) > consts.LEN_RFC_SLUG_MAX:
        raise ValueError(
            f"slug must be no greater than {consts.LEN_RFC_SLUG_MAX} characters long"
        )
    return slug
...
def validate_rfc_summary(summary: str) -> str:
    if len(summary) < consts.LEN_RFC_SUMMARY_MIN:
        raise ValueError(
            f"summary must be at least {consts.LEN_RFC_SUMMARY_MIN} characters long"
        )
    if len(summary) > consts.LEN_RFC_SUMMARY_MAX:
        raise ValueError(
            f"summary must be no greater than {consts.LEN_RFC_SUMMARY_MAX} characters long"
        )
    return summary
...
def validate_rfc_content(content: str) -> str:
    if len(content) < consts.LEN_RFC_CONTENT_MIN:
        raise ValueError(
            f"content must be at least {consts.LEN_RFC_CONTENT_MIN} characters long"
        )
    if len(content) > consts.LEN_RFC_CONTENT_MAX:
        raise ValueError(
            f"content must be no greater than {consts.LEN_RFC_CONTENT_MAX} characters long"
        )
    return content
...
def validate_revision_message(message: str) -> str:
    if len(message) < consts.LEN_REVISION_MSG_MIN:
        raise ValueError(
            f"message must be at least {consts.LEN_REVISION_MSG_MIN} characters long"
        )
    if len(message) > consts.LEN_REVISION_MSG_MAX:
        raise ValueError(
            f"message must be no greater than {consts.LEN_REVISION_MSG_MAX} characters long"
        )
    return message
...
class QuarantinedRFCSummary(BaseModel):
    ...
    rfc_slug: Annotated[str, AfterValidator(validate_rfc_slug)]
...
class RFCRevisionRequest(BaseModel):
    ...
    status: (
        Annotated[Literal["draft", "open"], AfterValidator(validate_rfc_status)] | None
    ) = None
    ...
    agent_contributors: (
        Annotated[list[str], AfterValidator(validate_agent_contributors)] | None
    ) = None
